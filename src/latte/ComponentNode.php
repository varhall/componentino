...
/**
 * {component name[:identifier][:method] [params]}
 */
class ComponentNode extends StatementNode
{
	public ExpressionNode $name;
	public ?ExpressionNode $identifier = null;
	...
	public static function create(Tag $tag): static
	{
		...
		if ($stream->tryConsume(':')) {
			$node->identifier = $tag->parser->parseExpression();
		}
	...
	public function print(PrintContext $context): string
	{
		...
		$fetchCode = $context->format(
			$this->name instanceof StringNode
				? '$ʟ_tmp = $this->global->uiControl->buildComponent(%node, (object) %node, %node);'
